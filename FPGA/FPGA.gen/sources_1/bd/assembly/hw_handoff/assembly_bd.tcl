...
# module references:
# clock_divider, debouncer, debouncer, encoder, encoder, not_gate, not_gate, pwm, pwm, spi
...
proc create_root_design { parentCell } {
  ...
  # Create instance: debouncer_pan, and set properties
  set block_name debouncer
  set block_cell_name debouncer_pan
  if { [catch {set debouncer_pan [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $debouncer_pan eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
    set_property -dict [ list \
  ...
  # Create instance: debouncer_tilt, and set properties
  set block_name debouncer
  set block_cell_name debouncer_tilt
  if { [catch {set debouncer_tilt [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $debouncer_tilt eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
    set_property -dict [ list \
  ...
  connect_bd_net -net b_0_1 [get_bd_ports tilt_b] [get_bd_pins encoder_tilt/b] [get_bd_pins xlconcat_1/In5]
  connect_bd_net -net b_0_2 [get_bd_ports pan_b] [get_bd_pins encoder_pan/b] [get_bd_pins xlconcat_1/In1]
  connect_bd_net -net clk_1 [get_bd_ports clk] [get_bd_pins clock_divider_0/clk]
  connect_bd_net -net clock_divider_0_clk_div [get_bd_pins clock_divider_0/clk_div] [get_bd_pins debouncer_pan/clk] [get_bd_pins debouncer_tilt/clk] [get_bd_pins encoder_pan/clk] [get_bd_pins encoder_tilt/clk] [get_bd_pins pwm_pan/clk] [get_bd_pins pwm_tilt/clk] [get_bd_pins spi_0/clk]
  connect_bd_net -net debouncer_pan_outp [get_bd_pins debouncer_pan/outp] [get_bd_pins encoder_pan/a] [get_bd_pins xlconcat_1/In2]
  connect_bd_net -net debouncer_tilt_outp [get_bd_pins debouncer_tilt/outp] [get_bd_pins encoder_tilt/a] [get_bd_pins xlconcat_1/In6]
  ...
  connect_bd_net -net pan_a_1 [get_bd_ports pan_a] [get_bd_pins debouncer_pan/inp] [get_bd_pins xlconcat_1/In0]
  ...
  connect_bd_net -net rst_0_1 [get_bd_ports rst] [get_bd_pins clock_divider_0/rst] [get_bd_pins debouncer_pan/rst] [get_bd_pins debouncer_tilt/rst] [get_bd_pins encoder_pan/rst] [get_bd_pins encoder_tilt/rst] [get_bd_pins spi_0/rst]
  ...
  connect_bd_net -net tilt_a_1 [get_bd_ports tilt_a] [get_bd_pins debouncer_tilt/inp] [get_bd_pins xlconcat_1/In4]
